...
class DownloadStation(base_api.BaseApi):
    ...
    def create_task_torrent(self, file_path, destination, create_list: bool = False) -> dict[str, object] | str:
        api_name = 'SYNO.DownloadStation' + self.download_st_version + '.Task'
        info = self.download_list[api_name]
        api_path = info['path']
        
        # Для type='file' нужен multipart/form-data
        # Формируем URL только с _sid, все остальные параметры в multipart
        url = ('%s%s' % (self.base_url, api_path)) + '?_sid=%s' % self._sid
        ...
        # Создаем multipart encoder согласно примеру запроса
        # Все параметры API в multipart, включая api, method, version
        # type должен быть "file" (в кавычках)
        # file должен быть ["torrent"] (JSON массив как строка)
        # destination в кавычках
        # create_list как false/true (boolean, не строка)
        encoder = MultipartEncoder({
            'api': api_name,
            'method': 'create',
            'version': str(info['maxVersion']),
            'type': '"file"',  # В кавычках как в примере
            'file': '["torrent"]',  # JSON массив как строка
            'destination': f'"{destination}"',  # В кавычках
            'create_list': str(create_list).lower(),  # false/true
        ...
        error_code = self.session._get_error_code(result)
        if error_code != 0:
            error_msg = result.get('error', {}).get('errors', result.get('error', {}))
            from .exceptions import DownloadStationError
            if hasattr(self.session, 'USE_EXCEPTIONS') and self.session.USE_EXCEPTIONS:
                raise DownloadStationError(error_code=error_code)
            return f'Ошибка API: {error_code} (детали: {error_msg})'
        return result
